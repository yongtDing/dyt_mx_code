...
/*32x64 AX_Channel Selection*/		
		uint16_t GPIOD_Activate_Array[16] = {	1, 2,  4,  8, 16, 32, 64, 128, 
																					32768, 16384, 8192, 4096, 2048, 1024, 512, 256};	
		uint16_t GPIOE_Activate_Array[16] = { 1, 2,  4,  8, 16, 32, 64, 128, 256, 
...
{	
		 uint8_t adc_rank[8] = {5,7,6,4,2,1,0,3};
			/*32x64 Data_SCAN*/
			...
			{	
					...
					{							
						...
						/*32x64 ADC_Value*/	
						adc_raw_value->value[AX][AY]      = GetAdcValue(0) / 2;
						adc_raw_value->value[AX][AY +  8] = GetAdcValue(3) / 2;				
						adc_raw_value->value[AX][AY + 16] = GetAdcValue(2) / 2;
						adc_raw_value->value[AX][AY + 24] = GetAdcValue(1) / 2;			
						adc_raw_value->value[AX][AY + 32] = GetAdcValue(5) / 2;
						adc_raw_value->value[AX][AY + 40] = GetAdcValue(6) / 2;	
						adc_raw_value->value[AX][AY + 48] = GetAdcValue(7) / 2;
						adc_raw_value->value[AX][AY + 56] = GetAdcValue(4) / 2;			
				}
			}		
}
