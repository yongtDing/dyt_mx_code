...
{	
			uint16_t value_t = 0;
			uint16_t adc_value_3 = 0, adc_value_2 = 0, adc_value_1 = 0, adc_value_0 = 0;
			/*32x64 Data_SCAN*/
			for(int8_t AY = 0; AY < 8; ++AY)
			{
					...
					{							
						...
						/*32x64 ADC_Value*/	
							...
							adc_value_3 = GetAdcValue(3);
							adc_value_2 = GetAdcValue(2);
							adc_value_1 = GetAdcValue(1);
							adc_value_0 = GetAdcValue(0);	
						
	
							if(adc_value_3 <= value_t)
							{
								adc_raw_value ->value[AX][AY] = 0;
							}else
								{
									adc_raw_value->value[AX][AY] = adc_value_3 - value_t;
								}
							if(adc_value_2 <= value_t)
							{
								adc_raw_value ->value[AX][AY + 8] = 0;
							}else
								{
									adc_raw_value->value[AX][AY + 8] = adc_value_2 - value_t;
								}
							if(adc_value_1 <= value_t)
							{
								adc_raw_value ->value[AX][AY + 16] = 0;
							}else
								{
									adc_raw_value->value[AX][AY + 16] = adc_value_1 - value_t;
								}								
							if(adc_value_0 <= value_t)
							{
								adc_raw_value ->value[AX][AY + 24] = 0;
							}else
								{
									adc_raw_value->value[AX][AY + 24] = adc_value_0 - value_t;
								}						
				...
				{			
							...
							{	
								...
								{									
									...
									adc_raw_value->voltage[AX][AY] = 1000000 / (Vref - (adc_raw_value->value[AX][AY] * Vref)
									/ (Vref - (adc_raw_value->sumAX[AY] - adc_raw_value->value[AX][AY]))) - 244;									
									if(adc_raw_value->voltage[AX][AY] > 255)
									{
										adc_raw_value->voltage[AX][AY] = 254;
									}
								}
								else
								{
...
/*------1-----*/
	...
	for(uint8_t ay = 0; ay < 2; ++ay)
	{
		...
		{
			adc_raw_value->voltage_29x28_display[ax][ay] = adc_raw_value->voltage[26 - ax][27 - ay];
		}	
	}
	for(uint8_t ay = 0; ay < 2; ++ay)
	{
		for(uint8_t ax = 2; ax < 6; ++ax)
		{
			adc_raw_value->voltage_29x28_display[ax][ay] = adc_raw_value->voltage[23 - ax][27 - ay];
		}	
	}
	for(uint8_t ay = 0; ay < 2; ++ay)
	{
		for(uint8_t ax = 6; ax < 8; ++ax)
		{
			adc_raw_value->voltage_29x28_display[ax][ay] = adc_raw_value->voltage[20 - ax][27 - ay];
		}	
	}
/*------2-----*/	
	for(uint8_t ay = 2; ay < 26; ++ay)
	{
		for(uint8_t ax = 0; ax < 24; ++ax)
		{
			adc_raw_value->voltage_29x28_display[ax][ay] = adc_raw_value->voltage[31 - ax][27 - ay];		
		}
	}
/*------3-----*/
	for(uint8_t ay = 9; ay < 15; ++ay)
	{
		for(uint8_t ax = 24; ax < 29; ++ax)
		{
			adc_raw_value->voltage_29x28_display[ax][ay] = adc_raw_value->voltage[31 - ax][34 - ay];		
		}	
	}
	for(uint8_t ay = 15; ay < 21; ++ay)
	{
		for(uint8_t ax = 24; ax < 29; ++ax)
		{
			adc_raw_value->voltage_29x28_display[ax][ay] = adc_raw_value->voltage[31 - ax][22 - ay];		
		}	
	}
/*------4-----*/
	for(uint8_t ay = 26; ay < 28; ++ay)
	{
		for(uint8_t ax = 0; ax < 2; ++ax)
		{
			adc_raw_value->voltage_29x28_display[ax][ay] = adc_raw_value->voltage[26 - ax][27 - ay];
