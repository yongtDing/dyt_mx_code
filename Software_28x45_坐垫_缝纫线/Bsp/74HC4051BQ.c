...
{	
			...
			for(uint8_t AY = 0; AY < 8; ++AY)
			{
					...
					{							
						...
						adc_raw_value->value[AX][63 - AY]      = GetAdcValue(6);
						adc_raw_value->value[AX][63 - AY -  8] = GetAdcValue(7);				
						adc_raw_value->value[AX][63 - AY - 16] = GetAdcValue(4);
						adc_raw_value->value[AX][63 - AY - 24] = GetAdcValue(5);			
						adc_raw_value->value[AX][63 - AY - 32] = GetAdcValue(3);
						adc_raw_value->value[AX][63 - AY - 40] = GetAdcValue(2);	
						adc_raw_value->value[AX][63 - AY - 48] = GetAdcValue(1);
						adc_raw_value->value[AX][63 - AY - 56] = GetAdcValue(0);
				}
			}		
}
...
void display_remapping(Hc4051AdcValue *adc_raw_value)
{
	...
	for(uint8_t ax = 0; ax <= 1; ++ax)
	{
		...
		{
			adc_raw_value->voltage_28x45_display[ax][ay] = adc_raw_value->voltage[27-ax][23 - ay];
		}
	}
	/*------2-----*/
	for(uint8_t ax = 2; ax <= 25; ++ax)
	{
		for(uint8_t ay = 0; ay <= 11; ++ay)
		{
			adc_raw_value->voltage_28x45_display[ax][ay] = adc_raw_value->voltage[27-ax][22 - ay + 16];
		}
		for(uint8_t ay = 12; ay <= 23; ++ay)
		{
			adc_raw_value->voltage_28x45_display[ax][ay] = adc_raw_value->voltage[27-ax][ay + 11 + 16];
		}
	}
//	/*------3-----*/
	for(uint8_t ax = 9; ax <= 14; ++ax)
	{
		for(uint8_t ay = 24; ay <= 26; ++ay)
		{
			adc_raw_value->voltage_28x45_display[ax][ay] = adc_raw_value->voltage[34-ax][34 - ay + 16];
		}
	}
	for(uint8_t ax = 9; ax <= 14; ++ax)
	{
		for(uint8_t ay = 27; ay <= 28; ++ay)
		{
			adc_raw_value->voltage_28x45_display[ax][ay] = adc_raw_value->voltage[34-ax][ay + 8 + 16];
		}
	}	
	
	for(uint8_t ax = 15; ax <= 20; ++ax)
	{
		for(uint8_t ay = 24; ay <= 26; ++ay)
		{
			adc_raw_value->voltage_28x45_display[ax][ay] = adc_raw_value->voltage[22-ax][34 - ay + 16];
		}
	}
	for(uint8_t ax = 15; ax <= 20; ++ax)
	{
		for(uint8_t ay = 27; ay<= 28; ++ay)
		{
			adc_raw_value->voltage_28x45_display[ax][ay] = adc_raw_value->voltage[22-ax][ay + 8 + 16];
		}
	}	
	
  /*------4-----*/	
	for(uint8_t ax = 26; ax <= 27; ++ax)
	{
		for(uint8_t ay = 0; ay <= 7; ++ay)
		{
			adc_raw_value->voltage_28x45_display[ax][ay] = adc_raw_value->voltage[27-ax][44 - ay + 16];
		}
