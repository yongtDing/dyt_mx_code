...
float amplification_coefficient = 80;
...
{	
			...
			for(int8_t AY = 0; AY < 8; ++AY)
			{
					...
					{							
						...
						adc_raw_value->value[AX][63 - AY]      = GetAdcValue(6) / 1;
						adc_raw_value->value[AX][63 - AY -  8] = GetAdcValue(7) / 1;				
						adc_raw_value->value[AX][63 - AY - 16] = GetAdcValue(4) / 1;
						adc_raw_value->value[AX][63 - AY - 24] = GetAdcValue(5) / 1;			
						adc_raw_value->value[AX][63 - AY - 32] = GetAdcValue(3) / 1;
						adc_raw_value->value[AX][63 - AY - 40] = GetAdcValue(2) / 1;	
						adc_raw_value->value[AX][63 - AY - 48] = GetAdcValue(1) / 1;
						adc_raw_value->value[AX][63 - AY - 56] = GetAdcValue(0) / 1;
				}
			}		
}
...
void display_remapping(Hc4051AdcValue *adc_raw_value)
{
	...
	for(uint8_t ax = 0; ax <= 2; ++ax)
	{
		...
		{
			adc_raw_value->voltage_30x24_display[ax][ay] = adc_raw_value->voltage[29-ax][7 - ay + 16];
		}
	}
	/*-------------2-------------*/	
	for(uint8_t ax = 3; ax <= 26; ++ax)
	{
		for(uint8_t ay = 0; ay <= 11; ++ay)
		{
			adc_raw_value->voltage_30x24_display[ax][ay] = adc_raw_value->voltage[29-ax][19 - ay + 16];
		}
	}	
	...
	for(uint8_t ax = 3; ax <= 26; ++ax)
	{
		...
		{
			adc_raw_value->voltage_30x24_display[ax][ay] = adc_raw_value->voltage[29-ax][ay + 8 + 16];
		}
	}	
	/*-------------3-------------*/	
	for(uint8_t ax = 27; ax <= 29; ++ax)
	{
		for(uint8_t ay = 0; ay <= 7; ++ay)
		{
			adc_raw_value->voltage_30x24_display[ax][ay] = adc_raw_value->voltage[29-ax][39 - ay + 16];
		}
	}	
}
