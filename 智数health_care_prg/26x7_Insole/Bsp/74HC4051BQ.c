...
{	
	...
	for(uint8_t AX = 0; AX < 26; AX++)
	{
		for(uint8_t y_index =0; y_index < 7; y_index++)
		{
				process_handle->adc_cali_value_display[AX][y_index] = process_handle->adc_cali_value[25-AX][y_index + 25];
			}
		}
}
